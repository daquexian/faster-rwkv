...
template <typename SRC, typename DST, bool affine> struct AffineStore {
  AffineStore(DST *y, int64_t row_size, int64_t channel_size,
              int64_t spatial_size, const DST *gamma, const DST *beta)
      : y(y), row_size(row_size), channel_size(channel_size),
        spatial_size(spatial_size), gamma(gamma), beta(beta) {}
...
#pragma unroll
    ...
    *(reinterpret_cast<cuda::layer_norm::PackType<DST, PackSize> *>(y) +
...
template <typename SRC, typename DST, bool affine> struct ScaleLoad {
  using LoadType = DST;
  ScaleLoad(const SRC *src, const SRC *gamma, int64_t row_size,
            int64_t channel_size, int64_t spatial_size)
      : src(src), gamma(gamma), row_size(row_size), channel_size(channel_size),
  ...
  __device__ void load(DST *dst, int64_t row, int64_t col) const {
    ...
    src_pack.storage =
        *(reinterpret_cast<const cuda::layer_norm::PackType<SRC, PackSize> *>(
              src) +
          packed_offset);
...
template <typename SRC, typename DST, bool affine> struct ChannelsLastStore {
  ChannelsLastStore(DST *y, const DST *gamma, const DST *beta,
                    int64_t spatial_size, int64_t channel_size,
                    int64_t num_groups)
      : y(y), gamma(gamma), beta(beta), spatial_size(spatial_size),
        c0(num_groups), c1(channel_size / num_groups) {}
  ...
  __device__ void store(const SRC *src, int32_t row, int32_t col) {
    ...
    if (affine) {
      gamma_pack.storage =
          *(reinterpret_cast<const cuda::layer_norm::PackType<DST, PackSize> *>(
                gamma) +
            gamma_beta_offset);
      beta_pack.storage =
          *(reinterpret_cast<const cuda::layer_norm::PackType<DST, PackSize> *>(
...
template <typename SRC, typename DST> struct ChannelsLastLoad {
  using LoadType = DST;
  ChannelsLastLoad(const SRC *src, int64_t spatial_size, int64_t channel_size,
                   int64_t num_groups)
      : src(src), spatial_size(spatial_size), c0(num_groups),
...
template <typename SRC, typename DST, bool affine> struct ChannelsLastStore {
  ...
  __device__ void store(const SRC *src, int32_t row, int32_t col) {
    ...
    const int32_t y_offset = (batch_idx * c0 * c1 * spatial_size +
                              spatial_idx * c0 * c1 + c0_idx * c1 + c1_idx) /
                             PackSize;
...
template <typename SRC, typename DST> struct ChannelsLastLoad {
  ...
  __device__ void load(DST *dst, int32_t row, int32_t col) const {
    ...
    int32_t c0_idx = row - batch_idx * c0;
    cuda::layer_norm::Pack<SRC, N> pack;
    const int32_t offset = (batch_idx * c0 * c1 * spatial_size +
                            spatial_idx * c0 * c1 + c0_idx * c1 + c1_idx) /
                           N;
  ...
  if (x.dtype() == weight.dtype() && x.dtype() == bias.dtype()) {
    if (x.dtype() == DType::kFloat32) {
      group_norm::GroupNormForwardGpu(
          instance_num, norm_size, channel_size, spatial_size, epsilon,
          x.data_ptr<float>(), weight.data_ptr<float>(), bias.data_ptr<float>(),
          out.data_ptr<float>(), true);
    } else if (x.dtype() == DType::kFloat16) {
      group_norm::GroupNormForwardGpu(
          instance_num, norm_size, channel_size, spatial_size, epsilon,
          x.data_ptr<half>(), weight.data_ptr<half>(), bias.data_ptr<half>(),
          out.data_ptr<half>(), true);
    } else {
      RV_CHECK(false);
    }
  } else {
