...
template <typename ThreadLock> class CachingAllocator final : public Allocator {
public:
...
private:
  ...
  // Bin structure of the corresponding BinSize. Pieces are stored in a linked
  // list. The Piece's prev and next are continuous with the current Piece in
  // physical memory.
  struct Piece {
    size_t size = 0;
    char *ptr = nullptr;
  ...
  // similar size, and these Pieces are arger than the size of bin
  //
  // BinAllocator has a set of Bin structures according to the binary multiple
  // increasing relation, which is used to quickly index and find the free Piece
  // of appropriate size when Allocate()
  //
  // The size of the smallest bin is 512 (512 is the smallest unit Allocated by
  // BinAllocator, and the memory size of all Allocated will be multiples of
  // 512, 512 is kCudaMemAllocAlignSize). The size of each Bin is twice the size
  // of the previous Bin, like
...
inline size_t MemAlignedBytes(size_t bytes, size_t alignment) {
  return RoundUp(bytes, alignment);
}

inline bool IsAlignedSize(size_t size, size_t alignment) {
...
CachingAllocator<ThreadLock>::CachingAllocator()
    : alignment_(kCudaAlignSize), total_memory_bytes_(0),
...
template <typename ThreadLock>
typename CachingAllocator<ThreadLock>::Piece *
CachingAllocator<ThreadLock>::AllocatePiece() {
...
template <typename ThreadLock>
typename CachingAllocator<ThreadLock>::Piece *
CachingAllocator<ThreadLock>::FindPiece(size_t aligned_size) {
  for (int32_t bin_num = BinNum4BinSize(aligned_size); bin_num < kBinNumSize;
       ++bin_num) {
    ...
    for (auto it = bin->pieces.begin(); it != bin->pieces.end(); ++it) {
      ...
      if (piece->size >= aligned_size) {
        ...
        piece->bin_num = kInvalidBinNum;
        piece->is_free = false;
        if (piece->size >= aligned_size * 2 ||
            piece->size - aligned_size >= kPieceSplitThreshold) {
...
template <typename ThreadLock>
void CachingAllocator<ThreadLock>::MergeNeighbourFreePiece(Piece *lhs,
                                                           Piece *rhs) {
...
template <typename ThreadLock>
bool CachingAllocator<ThreadLock>::AllocateBlockToExtendTotalMem(
    size_t aligned_size) {
  ...
  const size_t final_allocate_bytes =
      MemAlignedBytes(allocate_bytes, alignment_);
